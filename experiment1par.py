...
def process_computation(result):
    logging.debug("processing result %s", result)
    ...
    for ph in np.arange(phi1, phi2, phidelta):
        for th in np.arange(theta1, theta2, thetadelta):
            for w in np.arange(lambda1, lambda2, lambdadelta):
                list_of_args.append((ph, th, w, number_of_rays, aperture_collector))
    pool.map_async(compute,list_of_args, callback=process_computation)
